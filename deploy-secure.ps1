...
if ([string]::IsNullOrEmpty($env:JWT_SECRET)) {
    Write-Host "JWT_SECRET not set - using default (not recommended for production)" -ForegroundColor Yellow
} else {
    Write-Host "JWT_SECRET configured" -ForegroundColor Green
}

if ([string]::IsNullOrEmpty($env:MONGODB_URI)) {
    Write-Host "MONGODB_URI not set" -ForegroundColor Yellow
} else {
